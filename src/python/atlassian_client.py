#!/usr/bin/env python3
"""
Atlassian API Client — Shared module for Jira and Confluence operations.

Handles authentication (Basic auth via Cloud API), env loading, and provides
simple functions for common operations. Used by confluence_publisher.py and
the atlassian CLI script.
...
#   3. .env two levels up from this file (package root when bundled)
#   4. Legacy: ~/claude/projects/data_catalogue/.env
_this_dir = os.path.dirname(os.path.abspath(__file__))
_candidates = [
    os.environ.get('ATLASSIAN_ENV_PATH', ''),
    os.path.join(os.getcwd(), '.env'),
    os.path.join(_this_dir, '..', '..', '.env'),
    os.path.expanduser('~/claude/projects/data_catalogue/.env'),
]
ENV_PATH = next((p for p in _candidates if p and os.path.isfile(p)), _candidates[-1])

CONFLUENCE_SPACE_KEY = os.environ.get('CONFLUENCE_SPACE_KEY', 'DC')
CONFLUENCE_SPACE_ID = os.environ.get('CONFLUENCE_SPACE_ID', '203325447')
CONFLUENCE_ROOT_PAGE_ID = os.environ.get('CONFLUENCE_ROOT_PAGE_ID', '203325745')

JIRA_PROJECT = os.environ.get('JIRA_PROJECT_KEY', 'ITDT')
...
def _site_url():
    return _get_env().get('ATLASSIAN_SITE_URL', 'https://stars-ambulance.atlassian.net')
